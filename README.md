...
This repo ships a machine-readable map so an agent can orient without crawling the
tree. Start at [`AGENTS.md`](AGENTS.md), then read
[`agent-index.json`](agent-index.json): the concepts, components, and MCP tools
mapped to exact file paths and line spans. [`agent-graph.json`](agent-graph.json)
is the full structural graph (every symbol, import, and call edge), and
[`llms.txt`](llms.txt) is the discovery entry point. A benchmark of how much
context this saves is in
[`benchmarks/agent-index/STUDY.md`](benchmarks/agent-index/STUDY.md). The map is
generated by `scripts/` and CI fails if it drifts from the code.
